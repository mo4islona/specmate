...
- [ ] 1.1 State a profile's consequences in one object — the layout, whether the repository keeps the
...
- [ ] 1.2 Build a change path from a layout rather than a root, with one table mapping the two —
...
- [ ] 2.1 Add `tasks.change_layout` and generate the migration, whose backfill sets every
...
- [ ] 2.2 Split scaffolding out of `WorkspaceManager.provision`, so the working tree exists before
      the layout is known — D2, D4. Verify: `bun test packages/workspace`; provisioning alone no
      longer creates a change folder.
- [ ] 2.3 Pin the layout in `WorkspaceService.provision` after the convention is resolved, guarded on
...
- [ ] 3.1 Index from the working tree where the repository does not carry the folder, recording no
...
- [ ] 3.2 Index after every stage rather than only after a commit, keeping the deletion sweep on both
...
- [ ] 3.3 Restore an out-of-tree change folder from the index when the workspace does not carry it,
...
- [ ] 3.4 Refuse a discard whose workspace names no absolute path or no checkout, and hold every
...
- [ ] 4.1 Add the out-of-tree change folder to what a run is read to have changed, reporting a file
...
- [ ] 5.1 Read the approved summary from the artifact index instead of from a path on the branch —
...
- [ ] 7.1 Full suite green — `bun run test`, `bun run typecheck`, `bun run check`.
- [ ] 7.2 `bun run spec:validate` and `bun run spec:lint` pass over the change.

...
- [x] 4.2 Pin that the brief-completeness check fires for a proposal git does not report — REQ-1303,
...
- [x] 4.3 Pin that the declared-name convergence still sees a declaring run's artifacts under the
...
- [x] 6.1 Pin the surfaces for a task the repository carries no folder for: the diff route lists
      product code alone, and the files view draws no specification group — AC-1035, AC-1722.
      Verify: `bun test apps/api` and `bun run --cwd apps/web test`.

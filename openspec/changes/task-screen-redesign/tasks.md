...
- [x] 11.1 Add `DELETE /tasks/:id`: accept a task in any state, cancel it through the engine when
      it is not terminal, release the workspace before deleting, and leave the task intact when
      release fails — AC-1081, AC-1082, AC-1083. Verify: `bun test apps/api/test/app.test.ts`.
...
- [x] 11.4 Reshape a task-navigation row into a navigation link with a sibling overflow trigger;
      keep the trigger reachable by keyboard and touch, offer permanent deletion on every row, and
      place it last after a separator — AC-1805, AC-1806. Verify:
...
- [ ] 11.5 Add the confirmation: name the SpecMate data removed and the repository history kept,
      say the run is cancelled first when the task is still live, require the typed confirmation
      word, expose pending and failure states, remove the successful row, and return to the inbox
      when the deleted task is open — AC-1806, AC-1807, AC-1808. Verify:

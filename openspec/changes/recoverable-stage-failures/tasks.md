...
- [ ] 1.1 Have the exec result say whether the process that exited was the provider or a client
...
- [ ] 1.2 Add the backend failure to the stage failure vocabulary and raise it from
...
- [ ] 2.1 Replace the unions with one table over the vocabulary, each member carrying whether the
...
- [ ] 2.2 Give each member the sentence a human should read instead of its identifier, and render
...
- [ ] 2.3 Write the working note that keeps the analysis behind the table, in the shape
...
- [ ] 3.1 Add "does this image reference resolve here" to the backend interface, implemented for
...
- [ ] 3.2 Check the task's pinned image before dispatching a stage and, when it does not resolve,
...
- [ ] 3.3 Fail the stage naming the unresolvable image, leaving the stored pin as it was, when
...
- [ ] 4.1 Take the change name declared by the run's own result into the write-scope check, for a
...
- [ ] 4.2 Say in the role file which folder to write into and that the declared name is applied
...
- [ ] 4.3 Name the change folder in the assembled prompt as an instruction rather than only as a
...
- [ ] 5.1 Let a failed or declined attempt's reason and detail leave the executor and reach the
      ...
      (AC-248, AC-249).
- [ ] 5.2 Carry it across a re-dispatch too, so an attempt the engine dispatches after a failed
...
- [ ] 6.1 Continue the declined attempt's own session on the retry — declined being what §2's
...
- [ ] 7.1 Fail the task immediately, without re-dispatching, when §2's table says re-running
...
- [ ] 7.2 Stop the runner's own second attempt for the same class, so the two caps agree
...
- [ ] 8.1 Run the full suite: `bun run test`, `bun run typecheck`, `bun run lint`.
- [ ] 8.2 Run `bun scripts/lint-spec-ids.ts` and confirm no duplicate or dangling ID.
- [ ] 8.3 Record in `deploy/RUNBOOK.md` that a runner-image rebuild no longer strands tasks, and

...
which group it is in. The list SHALL also name the commit it was computed against, so that a
reader can tell one comparison from the next and can drop what it recorded about an older one.

The API SHALL return the unified diff for one named file from that same comparison, for a path in
either group. That read SHALL accept how much context to return around each hunk, bounded by a
maximum the API enforces; a width the API cannot honour SHALL be answered with the widest it
will serve rather than as an error, and a width past the file's length SHALL return the file's
whole text as the hunk's context.

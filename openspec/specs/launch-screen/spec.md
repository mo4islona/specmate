...
Defines the one screen that exists before a task does: what the owner is asked for in order to
launch work, what the system settles on their behalf, and what it shows them about where that work
is about to go. Split out of `operator-ui`, which keeps every screen that presupposes a task.

Requirements here keep the IDs they were allocated under `operator-ui`; new ones come from this
capability's own band.

...
### Requirement: REQ-701 — One isolated working tree per task
...
#### Scenario: AC-701 — Two tasks against one repository
...
#### Scenario: AC-702 — The base branch is never written
...
### Requirement: REQ-702 — Provisioning is idempotent and self-repairing
...
#### Scenario: AC-703 — Workspace requested twice
...
#### Scenario: AC-704 — Interrupted provisioning
...
#### Scenario: AC-705 — Working tree no longer matches its registration
...
### Requirement: REQ-704 — One shared local copy per repository
...
#### Scenario: AC-709 — Second task on the same repository
...
#### Scenario: AC-710 — Concurrent provisioning
...
#### Scenario: AC-711 — Two tasks arrive before the first copy exists
...
#### Scenario: AC-712 — Copying is interrupted
...
#### Scenario: AC-713 — A slow copy is not mistaken for an abandoned one
...
#### Scenario: AC-714 — Waiting request gives up
...
### Requirement: REQ-706 — Stage output is committed before the stage is reported complete
...
#### Scenario: AC-718 — Stage changed files
...
#### Scenario: AC-719 — Stage changed nothing
...
#### Scenario: AC-720 — Crash after committing
...
### Requirement: REQ-707 — Runner scratch never enters a commit
...
#### Scenario: AC-721 — Result file left in the workspace
...
#### Scenario: AC-722 — Repository ignore rules are not touched

- **WHEN** scratch exclusions are applied to a workspace
- **THEN** the repository's own ignore files SHALL be unchanged

### Requirement: REQ-708 — Committed artifacts are indexed for display
...
### Requirement: REQ-709 — Workspaces read from the remote and never write to it
...
#### Scenario: AC-726 — Write to the remote attempted
...
### Requirement: REQ-710 — Release destroys the working tree and keeps the history
...
#### Scenario: AC-728 — Task reaches a terminal state
...
#### Scenario: AC-729 — Release of an active task

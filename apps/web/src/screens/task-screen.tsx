...
export function TaskScreen({ taskId }: TaskScreenProps) {
  ...
  const detail = useQuery({ queryKey: queryKeys.task(taskId), queryFn: () => getTask(taskId) })
  const timeline = useQuery({
    queryKey: queryKeys.events(taskId),
    queryFn: () => listEvents(taskId),
        ...
        <section className="panel attention-pulse border-amber/45 p-4 sm:p-5">
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
            ...
            <button
              ...
              disabled={approve.isPending || redirect.isPending || rework.isPending}
            >
            ...
            <div className="flex min-w-48 flex-col gap-2">
              {currentGate.redirect && (
                <button
                  ...
                  disabled={!gateComment.trim() || redirect.isPending}
              ...
              {reworkTargets.length > 0 && (
                <>
                  ...
                  <button
                    ...
                    disabled={!gateComment.trim() || !reworkTarget || rework.isPending}

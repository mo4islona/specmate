...
describe('conversation message item', () => {
  ...
  test('names who is speaking, so the thread reads as a conversation', () => {
    expect(renderToStaticMarkup(<ConversationMessageItem message={message()} />)).toContain('guide')
    expect(
      renderToStaticMarkup(<ConversationMessageItem message={message({ role: 'owner' })} />),
    ).toContain('you')
  })
